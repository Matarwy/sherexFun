...
import {
  ExtensionType, Mint, MintLayout, TOKEN_2022_PROGRAM_ID, TOKEN_PROGRAM_ID, TransferFeeConfig, TransferFeeConfigLayout, unpackMint
...
export function getTransferFeeConfig(mint: Mint): TransferFeeConfig | null {
  ...
  if (extensionData !== null) {
    return TransferFeeConfigLayout.decode(Uint8Array.from(extensionData))
  } else {
    return null
  }
}
  ...
  try {
    if (connection) {
      ...
      if (space === 82 || space >= 165) {
        ...
        try {
          if (info && MintLayout.decode(Uint8Array.from(info?.data)).freezeAuthorityOption === 1) tags.push('hasFreeze')
        } catch {
