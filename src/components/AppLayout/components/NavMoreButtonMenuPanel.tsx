...
export function NavMoreButtonMenuPanel() {
  ...
  return (
    <MenuList>
      <Box py={3}>
        {/* <MenuItem>
          <Link as={NextLink} _hover={{ textDecoration: 'none' }} w="full" href={`/birthpad/profile?wallet=${publicKey ? publicKey.toBase58() : ''}${referrerQuery.replace('?', '&')}`}>
            <HStack>
      ...
      <Flex mb={-1} mt={1} py={2} justifyContent={'space-around'} color={colors.textSecondary} bg={colors.backgroundTransparent07}>
        <Link as={NextLink} href="https://x.com/sherexcoin" isExternal>
          <TwitterMediaIcon />
        </Link>
        <Link as={NextLink} href="https://t.me/sherexcoin" isExternal>
          <TelegrameMediaIcon />
        </Link>
