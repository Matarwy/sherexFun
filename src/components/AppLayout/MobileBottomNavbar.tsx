import { Box, ColorMode, Menu, MenuButton, SimpleGrid, Text, useColorMode, VStack } from '@chakra-ui/react'
import Link from 'next/link'
import { useRouter } from 'next/router'
import { ReactNode } from 'react'
import { useTranslation } from 'react-i18next'

import BirthpadPageThumbnailIcon from '@/icons/pageNavigation/BirthpadPageThumbnailIcon'
...
export function MobileBottomNavbar() {
  ...
  const birthpadHref = '/birthpad'
  const isBirthpadActive = pathname.includes(birthpadHref)
  ...
  return (
    ...
    >
      <BottomNavbarItem
        href={birthpadHref}
        text={t('birthpad.title')}
        icon={(colorMode) => <BirthpadPageThumbnailIcon colorMode={colorMode} isActive={isBirthpadActive} />}
        isActive={isBirthpadActive}
      />
      <BottomNavbarItem
        href={swapHref}
        text={t('swap.title')}
