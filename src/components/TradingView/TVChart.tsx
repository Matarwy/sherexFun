...
import { AppColorMode, AppTheme, THEME_NAMES, Themes } from './TvTheme'
...
export default function TVChart({
  ...
  useEffect(() => {
    ...
    const checkData = async () => {
      try {
        const { data } = await axiosInstance.get(`${useBirthpadStore.getState().historyHost}/kline?poolId=${poolId}&interval=1m&limit=1`)
