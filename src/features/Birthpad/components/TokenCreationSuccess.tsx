  ...
  return (
    <Box mx="auto">
      ...
      <Text textAlign="center" color={colors.lightPurple} mb={3}>
        {t('birthpad.successfully_created_token')}
      </Text>
      <Flex
        px={5}
        py={3}
      ...
      <Flex direction="column" maxWidth="442px">
        ...
        >
          <Box>
            <Flex fontWeight="medium" gap={1} alignItems="center">
              <WarningIcon />
              <Text>{t('birthpad.configure_vesting')}</Text>
            </Flex>
            <Text fontSize="sm" color="#C4D6FF80">
              {t('birthpad.vesting_description')}
            </Text>
          </Box>
          <Link
            as={NextLink}
        ...
        >
          <Box>
            <Text fontWeight="medium">{t('birthpad.view_token')}</Text>
            <Text fontSize="sm" color="#C4D6FF80">
              {t('birthpad.token_details_description')}
            </Text>
          </Box>
          <Link
            as={NextLink}
            href={`/birthpad/token?mint=${token.mint}${referrerQuery}`}
            display="contents"
            shallow
            color={colors.lightPurple}
          >
