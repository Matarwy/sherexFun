import { Box, Flex, Grid, Image, Skeleton, Text, Tooltip, useClipboard, useColorMode } from '@chakra-ui/react'
...
const TokenCard = ({ token }: { token: MintInfo }) => {
  ...
  return (
    <Box
      ...
      cursor="pointer"
      onClick={() => {
        router.push(`/birthpad/token?mint=${token.mint}${queryReferrer}`)
      }}
      _before={
        token.finishingRate >= HEATING_RATE
          ? isLight
    ...
    >
      <Grid templateRows="auto 1fr" gap={2}>
        <Grid templateColumns="1fr auto" alignItems="center">
          ...
          <Flex color={colors.textBirthpadLink} alignItems="center" gap="1">
            {token.defaultCurve ? null : (
              <Tooltip
                hasArrow
                placement="top"
                label={'This token has custom BirthPad parameters set by the creator. Make sure to check the token page for full details.'}
              >
                <Box>
                  <Curve />
                </Box>
